...
def create_model(
    ...
    learning_rate_decay_factor = (final_learning_rate / initial_learning_rate)**(1/300)
    ...
    optimizer = tf.keras.optimizers.Adam(learning_rate = 1e-4) #lr_exponential_decay_schedule)
...
df = pd.read_csv("nn_q3_features.csv")
...
df.drop('background_counts', axis=1, inplace=True)
df.drop('background_area_ratio', axis=1, inplace=True)
...
for outer_fold, (inner_idx, outer_idx) in enumerate(outer_kf.split(data, labels_one_hot)):
    ...
    for inner_fold, (train_idx, val_idx) in enumerate(inner_kf.split(inner_data, inner_labels)):
        ...
        train_f1_score = history.history['f1_score'][best_epoch]
        ...
        val_f1_score = history.history['val_f1_score'][best_epoch]
        ...
        # test_performance is [loss, recall, precision, f1_score, accuracy, auc]
        test_loss = test_performance[0]
        test_recall = test_performance[1]
        test_precision = test_performance[2]
        test_f1_score = test_performance[3]
        test_accuracy = test_performance[4]
        test_auc = test_performance[5]
