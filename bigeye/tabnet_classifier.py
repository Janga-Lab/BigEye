...
for outer_fold, (inner_idx, outer_idx) in enumerate(outer_kf.split(data, labels_one_hot)):
    ...
    for inner_fold, (train_idx, val_idx) in enumerate(inner_kf.split(inner_data, inner_labels)):
        ...
        model = None
        model = TabNetClassifier(n_classes = 5, out_activation = "softmax", **tabnet_params)
...
print("\n" + "="*70)
print("NESTED CROSS-VALIDATION RESULTS - TabNet")
print("="*70)
...
with open('tabnet_nested_cv_results.pickle', 'wb') as handle:
    pickle.dump(
        results,
        handle,
        protocol=pickle.HIGHEST_PROTOCOL
...
print("\nResults saved to: tabnet_nested_cv_results.pickle")

# Save best model
if best_model is not None:
    # Save as TensorFlow SavedModel format (for subclassed models)
    model_dir = f'{classifier_name}_best_model'
    best_model.save(model_dir, save_format='tf')
    
    # Also save weights separately
    weights_filename = f'{classifier_name}_best_model_weights.h5'
    ...
    print(f"\nBest model saved to: {model_dir}/ (SavedModel format)")
